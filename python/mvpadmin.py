...
if not(options.cmd_status or options.cmd_kill or options.cmd_launch_filename):
  parser.print_help()
  exit(1)
...
def printStatusReport(status_report):
  ...
  for i in status_report.actives:
    print "{0}, {1} @{2}: {3:.2f}%".format(i.job.render.col, i.job.render.row, i.job.render.level, i.status * 100)
