...
Footprint Footprint::construct_from_paths(std::string const& image_path, 
                                          ...
                                          vw::Vector2 const& alt_limits)
{
  ...
  boost::scoped_ptr<vw::DiskImageResource> rsrc(vw::DiskImageResource::open(image.image_path()));
  vw::Vector2i image_size(rsrc->cols(), rsrc->rows());

  std::vector<vw::Vector2> fp_points(8);

  vw::camera::PinholeModel camera(image.camera_path());
  ...
  return Footprint(image, image_size, fp_points);
}

int Footprint::equal_resolution_level() const {
...
int Footprint::equal_density_level(int tile_size) const {
  vw::BBox2 fp_bbox(bounding_box());
