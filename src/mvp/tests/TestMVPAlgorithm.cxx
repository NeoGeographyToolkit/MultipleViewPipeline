...
class MVPAlgoFootprintTest : public ::testing::TestWithParam<bool> {
  protected:
    virtual void SetUp() {
      // tile_size
      tile_size = 64;
      ...
      if (GetParam()) {
        impl.reset(new MVPAlgoOctave(crops, MVP_OCTAVE_FOOTPRINT_FCN));
      } else {
        impl.reset(new MVPAlgoFootprint(crops));
      }
    }
    ...
    boost::shared_ptr<MVPAlgorithm> impl;
};

TEST_P(MVPAlgoFootprintTest, operator) {
  ...
  for (int i = 0; i < this->tile_size; i += validation_divisor) {
    for (int j = 0; j < this->tile_size; j += validation_divisor) {
      ...
      result.update(i, j, (*(this->impl))(MVPAlgorithmVar(), crop(this->georef, i, j), MVPAlgorithmOptions()));
...
INSTANTIATE_TEST_CASE_P(instance, MVPAlgoFootprintTest, ::testing::Bool());
#else
INSTANTIATE_TEST_CASE_P(instance, MVPAlgoFootprintTest, ::testing::Values(false));
#endif
